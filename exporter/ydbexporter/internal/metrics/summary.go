...
func (g *summary) tableName(config *config.TableConfig) string {
	return config.Name + "_summary"
}

func (g *summary) createTableOptions(config *config.TableConfig) []options.CreateTableOption {
	opts := []options.CreateTableOption{
		...
		options.WithColumn("valueAtQuantiles", types.Optional(types.TypeJSONDocument)),
...
func (g *summary) createRecords(resourceMetrics pmetric.ResourceMetrics, scopeMetrics pmetric.ScopeMetrics, metric pmetric.Metric) ([]types.Value, error) {
	...
	for i := 0; i < dataPoints.Len(); i++ {
		...
		quantilesValues, err := json.Marshal(convertValuesAtQuantiles(dp.QuantileValues()))
		if err != nil {
			return nil, err
		}
		...
		record := types.StructValue(
			...
			types.StructFieldValue("valueAtQuantiles", types.JSONDocumentValueFromBytes(quantilesValues)),
		)
...
type ValueAtQuantiles struct {
	Quantile float64 `json:"quantile"`
	Value    float64 `json:"value"`
}

func convertValuesAtQuantiles(valuesSlice pmetric.SummaryDataPointValueAtQuantileSlice) []ValueAtQuantiles {
	var values []ValueAtQuantiles
